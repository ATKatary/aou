...
interface AOUCanvasProps extends React.PropsWithChildren<any> {
    meshes: number[] // placeholder for now to test state management, actual will be a meshDataType[]
}
export default function AOUCanvas({meshes, ...props}: AOUCanvasProps) {
    ...
    return (
        ...
        >
            ...
            <OrbitControls maxDistance={7} ref={orbitRef}/>
            ...
            <group>
                {meshes.map((i, mesh) => {
                    return (
                        <AOUMesh orbitRef={orbitRef} key={mesh} position={[i*1.5, 0.5, 0]}/>
                    )
                })}
